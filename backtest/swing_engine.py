"""
Swing Low Backtest Engine.

A dedicated, clean backtest engine for the Swing Low Reversal Strategy.

Flow:
  1. SwingLowStrategy generates BUY signals at each confirmed swing low.
  2. For each bar:
     a. If flat and a BUY signal fires -> enter long.
     b. If in position -> check stop loss (1 ATR) and take profit (2 ATR).
  3. Track equity curve and trade log.
...
from strategy.swing_low_strategy import SwingLowStrategy, SwingSignal
...
class SwingBacktestEngine:
    """
    Event-driven backtest engine for the Swing Low strategy.
    ...
    commission : float
        Per-trade commission (default 0 for Schwab equities).
    slippage : float
        Simulated slippage per share (default $0.01).
    swing_window : int
    ...
    def run(self, df: pd.DataFrame) -> SwingBacktestResult:
        """
        ...
        SwingBacktestResult
        """
        logger.info("Starting Swing Low backtest on %d bars...", len(df))

        # ── Generate signals ────────────────────────────────────
        strategy = SwingLowStrategy(
        ...
        for i in range(len(df)):
            ...
            if np.isnan(atr_val) or atr_val <= 0:
                equity_curve.append(equity if position is None else
                                    equity + (close - position["entry_price"]) * position["quantity"])
                continue
            ...
            if position is not None:
                ...
                # Stop loss hit? (price went below stop)
                if low <= position["stop_loss"]:
                    exit_price = position["stop_loss"]
                    exit_reason = "Stop Loss (1 ATR)"

                # Take profit hit? (price went above target)
            ...
            if position is None and i in signal_by_bar:
                ...
                if ps.shares > 0:
                    ...
                    logger.debug(
                        "BUY @ %.2f  SL=%.2f  TP=%.2f  qty=%d  swing_low=%.2f",
        ...
        logger.info(
            "Swing Low backtest complete: %d trades, final equity=%.2f, return=%.2f%%",
    ...
    def _compute_metrics(
        self, trades: List[SwingTrade], equity: pd.Series
    ) -> dict:
